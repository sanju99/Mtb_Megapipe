...
rule get_input_FASTQ_files:
    ...
    shell:
        ...
        fastq-dump --split-files {params.run_ID} --outdir {params.run_out_dir}
        """
...
rule trim_adapters:
    ...
    # conda:
    #     f"{primary_directory}/envs/Matthias_pipeline.yaml"
    params:
        min_read_length = config["min_read_length"]
    shell:
        """                
        source activate /home/sak0914/anaconda3/envs/Matthias_pipeline
        
        fastp -i {input.fastq1} \
              -o {output.fastq1_trimmed} \
              -h {output.fastp_html} \
              -j {output.fastp_json} \
...
rule kraken_classification:
    ...
    shell:
        ...
        kraken2 --db {params.kraken_db} \
                ...
                --classified-out {output.fastq1_trimmed_classified} \
                --output {output.kraken_classifications} \
                {input.fastq1_trimmed}
        """
...
rule fastlin_typing:
    ...
    shell:
        ...
        fastlin -d {output.fastlin_dir} -b {params.fastlin_barcodes} -o {output.fastlin_output} -x 250
        """
...
rule align_reads_mark_duplicates:
    ...
    shell:
        ...
        bwa mem -M -R "@RG\\tID:{wildcards.run_ID}\\tSM:{wildcards.sample_ID}" -t 8 {params.ref_genome} {input.fastq1_trimmed_classified} > {output.sam_file}
...
rule pilon_variant_calling:
    ...
    shell:
        """
        source activate /home/sak0914/anaconda3/envs/Matthias_variant_calling
        ...
        bcftools norm --rm-dup none --fasta-ref {params.ref_genome} {output.vcf_file} | bcftools sort > {output.vcf_file_full}
        """
