...
def get_all_drug_resistances(df):
    ...
    for drug in np.sort(who_catalog_V2['drug'].unique()):

        # get only Group 1-2 resistance mutations
        R_assoc_variants = who_catalog_V2.loc[who_catalog_V2['confidence'].str.contains('Assoc w R')].query("drug==@drug").variant.values
        ...
        negating_muts = who_catalog_V2.dropna(subset="Comment").query("drug==@drug & Comment.str.contains('Abrogates')").variant.values
...
def make_susceptible_dataframe():
    ...
    resistance_df['Drug'] = np.sort(who_catalog_V2['drug'].unique())
...
# for VCFs that weren't generated in the typical way. These were generatd by pairwise aligning an assembly and H37Rv, then calling variants. So many of the QC columns aren't there
if sum(pd.isnull(df['FILTER'])) == len(df):
    ignore_QC = True
else:
    ignore_QC = False
...
# these variants typically have high MQ and DP, so they are very likely real. Seems to be a sequencing artifact / issue
...
if not ignore_QC:
    ...
    # use all variants, don't do any QC filtering
    df_highConf_muts = df.copy()
