...
# for VCFs that weren't generated in the typical way. These were generated by pairwise aligning an assembly and H37Rv, then calling variants. So many of the QC columns aren't there
if sum(pd.isnull(df['FILTER'])) == len(df):
    ignore_QC = True
else:
    ignore_QC = False

# print(f"ignore_QC: {ignore_QC}")
...
# set AF = 0.25 for long indels (difference > 10). For short indels, use AF = 0.75 as usual
if not ignore_QC:
    ...
    df_highConf_muts = pd.concat([# long deletions
                                    df.loc[(df['REF'].str.len() - df['ALT'].str.len() > 10) & 
                                           (~df['FILTER'].str.contains('|'.join(['Del', 'LowCov'])))
                                            ].query("~(AF <= 0.25) & ~(DP < 5) & ~(MQ < 30) & ~(DC < 5)"), # deletion
        
                                    # long insertions
                                    df.loc[(df['ALT'].str.len() - df['REF'].str.len() > 10) & 
                                           (~df['FILTER'].str.contains('|'.join(['Del', 'LowCov'])))
                                            ].query("~(AF <= 0.25) & ~(DP < 5) & ~(MQ < 30) & ~(IC < 5)"), # insertion
    
                                    # short deletions: 
                                    df.loc[(df['REF'].str.len() - df['ALT'].str.len() <= 10) &
                                           (df['REF'].str.len() - df['ALT'].str.len() > 0) &
                                           (~df['FILTER'].str.contains('|'.join(['Del', 'LowCov'])))
                                            ].query("~(AF <= @AF_thresh) & ~(DP < 5) & ~(MQ < 30) & ~(DC < 5)"), # deletion
                                  
                                    # short insertions: 
                                    df.loc[(df['ALT'].str.len() - df['REF'].str.len() <= 10) &
                                           (df['ALT'].str.len() - df['REF'].str.len() > 0) &
                                           (~df['FILTER'].str.contains('|'.join(['Del', 'LowCov'])))
                                            ].query("~(AF <= @AF_thresh) & ~(DP < 5) & ~(MQ < 30) & ~(IC < 5)"), # insertion
