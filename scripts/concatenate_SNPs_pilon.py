...
parser.add_argument('--AF', type=float, dest='AF_THRESH', default=0.75, help='Alternative allele frequency threshold (exclusive) to consider variants present')
...
parser.add_argument('--BQ', type=float, dest='BQ_THRESH', default=20, help='Base quality threshold (inclusive) to consider SNPs high quality')
parser.add_argument('--DP', type=float, dest='DP_THRESH', default=5, help='Total read depth threshold (inclusive) to consider SNPs high quality')
...
# optional arguments
AF_THRESH = cmd_line_args.AF_THRESH # variants with an AF > AF_THRESH are considered present. AF ≤ 0.25 is absent. Everything else is missing
MQ_THRESH = cmd_line_args.MQ_THRESH
BQ_THRESH = cmd_line_args.BQ_THRESH
DP_THRESH = cmd_line_args.DP_THRESH
...
def check_is_snp(record):
    ...
    if 'IMPRECISE' in record.INFO.keys() or 'SVTYPE' in record.INFO.keys():
        ...
        # return True only if it is a SNP (REF and ALT length are the same and the alleles are different). Might be unnecessary because we already filtered with bcftools, but just in case.
...
def allele_category(record, qualThresh=10, presentThresh=0.75, DP_thresh=5, MQ_thresh=40, BQ_thresh=20):
    '''
    ...
    if 'DP' in record.INFO.keys():
        if record.INFO['DP'] < DP_thresh:
            return 'missing'

    if 'MQ' in record.INFO.keys():
        if record.INFO['MQ'] < MQ_thresh:
            return 'missing'
    ...
    if af <= 0.25:
        return "ref"
    elif af > 0.25 and af <= presentThresh:
        return "missing"
    # present depends on the threshold passed in
    elif af > presentThresh:
        return "alt"

    # if nothing has been returned at this point, then the variant is high quality (there are no REF = ALT records in the input VCF files, so return the alternative variant)
    return "alt"



def introduce_snps_single_seq(fName, h37Rv_region, START, END, qualThresh=10, presentThresh=0.75, DP_thresh=5, MQ_thresh=30, BQ_thresh=20):
    ...
    for record in records:
        ...
        # only process SNPs with this script. Everything else will be considered reference.
        if check_is_snp(record):
            ...
            single_allele_type = allele_category(record, qualThresh, presentThresh, DP_thresh, MQ_thresh, BQ_thresh)
...
print(f"Considering AFs > {AF_THRESH} as present with DP ≥ {DP_THRESH}, MQ ≥ {MQ_THRESH}, and BQ ≥ {BQ_THRESH} and writing {len(file_paths)} SNP concatenates to {OUT_FILE}")
...
with open(OUT_FILE, "w+") as file:
    ...
    for i, fName in enumerate(file_paths):
        ...
        seq_with_snps = introduce_snps_single_seq(fName, h37Rv_region, START, END, qualThresh=10, presentThresh=AF_THRESH, DP_thresh=DP_THRESH, MQ_thresh=MQ_THRESH, BQ_thresh=BQ_THRESH)
