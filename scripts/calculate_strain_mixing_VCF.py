import vcf, os, sys, argparse, itertools
import pandas as pd
import numpy as np
...
parser.add_argument('--num_mixed_lineages', type=int, dest='num_mixed_lineages', default=2, help='Number of mixed lineages to consider. F2 = 2 mixed lineages, but you can specify larger numbers to include more into the calculation')
...
num_mixed_lineages = cmd_line_args.num_mixed_lineages

output_file = os.path.join(os.path.dirname(allele_fractions_by_lineage), f"F{num_mixed_lineages}_Coll2014.txt")
	...
	for record in vcf_reader:
		...
		if 'SVTYPE' not in record.INFO.keys():
			...
			if ( reference_position in list( lineage_defining_SNPs.position ) ) and check_call_is_SNP(ref_allele , alt_alleles):
				...
				if (BQ > 20) and (MQ > 30) and (DC == 0) and (IC == 0) and (VD >= 10): #SNP passed filtering criteria!
...
top_N_lineages = get_top_N_distinct_lineages(df_allele_fractions, num_mixed_lineages)
print(f"Top lineages: {top_N_lineages}")
